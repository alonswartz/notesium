...
setup_file() {
    ...
    run cp $BATS_TEST_DIRNAME/fixtures/0c8bea98.md /tmp/notesium-test-corpus/
    export NOTESIUM_DIR="/tmp/notesium-test-corpus"
    export PATH="$(realpath $BATS_TEST_DIRNAME/../):$PATH"
}

teardown_file() {
    run rm /tmp/notesium-test-corpus/0c8bea98.md
...
@test "orphan2: links file" {
    run notesium.sh links 0c8bea98.md
    echo "$output"
    [ $status -eq 0 ]
    [ "${lines[0]}" == '' ]
}
...
@test "orphan2: links outgoing" {
    run notesium.sh links --outgoing 0c8bea98.md
    echo "$output"
    [ $status -eq 0 ]
    [ "${lines[0]}" == '' ]
}
...
@test "orphan2: links incoming" {
    run notesium.sh links --incoming 0c8bea98.md
    echo "$output"
    [ $status -eq 0 ]
    [ "${lines[0]}" == '' ]
}
